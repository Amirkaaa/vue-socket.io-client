...
export default class EventEmitter {
  ...
  /**
   * Register new event listener with vue component instance
   * @param event
   * @param callback
   * @param component
   */
  addEventListener(event, callback, component) {
   ...
   * @param component
   */
  removeEventListener(event, component) {
    if (this.listeners.has(event)) {
      const listeners = this.listeners.get(event).filter(listener => listener.component !== component);

      listeners.length > 0 ? this.listeners.set(event, listeners) : this.listeners.delete(event);
  ...
  /**
   * Broadcast incoming event to vue components
   * @param event
   * @param args
   */
  emit(event, args) {
